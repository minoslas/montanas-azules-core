...
import { IContextoSimulacion, TipoTarea, PrioridadTarea, EstadoTarea, EstadoIA, TipoBloque } from "../model/Tipos";
...
export class Simulador {
    ...
    public procesarTick(): void {
        ...
        this.entidades.forEach(entidad => {
            ...
            else if (entidad.getCargaActual() >= entidad.getCapacidadMax() && entidad.estado !== EstadoIA.BUSCAR_RECURSO) {
    ...
    private asignarRecursoVital(entidad : Draconiano) {
        ...
        if(almacenDestino){
            ...
            console.log(`[SIM] Emergencia vital: Redirigiendo a ${entidad.nombre} a por ${recursoBuscado}.`);
        } else {
            // Crisis en la colonia: No hay recursos
            if (this.tickActual % 10 === 0) {
                console.warn(`[SIM-ALERTA] ¡Las reservas de ${recursoBuscado} están vacías! ${entidad.nombre} no tiene a dónde ir.`);
            }
        }
    }
